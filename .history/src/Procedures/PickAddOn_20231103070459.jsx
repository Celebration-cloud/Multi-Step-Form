...
function PickAddOn({addOn, planner}) {
  ...
  const handleTick = (e, odding) => {
    const {name, value, checked} = e.target
    setSelected(prev => [...prev, e.target.checked])
    if (selected === true) {
      setView((prev) => [...prev, odds])
    } else {
  ...
  return (
    ...
    <div style={{display: 'flex', justifyContent:'center'}}>
      {pickAddOnList?.map((odds)=> (
        <div key={odds.id}>
          <input type="checkbox" checked={selected} onChange={(e) => setSelected(e.target.checked)} value={odds} />
          <div>
            <h6>{odds.name}</h6>
