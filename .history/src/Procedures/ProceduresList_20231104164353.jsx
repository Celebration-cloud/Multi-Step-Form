...
function PersonalInfo({store}) {
  ...
  const handlePersonalInfo = (e) => {
    if(Number(personalData.phoneNumber).length < 14) return;
    const {name, value, type} = e.target
    setPersonalData(prev => {return {...prev, [name]: type === 'tel' ? Numbervalue.replace(/[^0-9]/g, '') : value}})
  ...
  return (
    <div className="mobileTab">
      ...
      <input
        ...
        id="phoneNumber"
        name="phoneNumber"
        value={Number(personalData.phoneNumber)}
        onChange={handlePersonalInfo}
        type="tel"
