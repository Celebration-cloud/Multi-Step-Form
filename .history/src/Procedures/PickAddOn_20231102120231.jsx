...
function PickAddOn({addOn}) {
  ...
  //   setPick(event.target.checked)
  //   if(pick === false)return;
  // }
const handlePicking = (dap, check) => {
  setPick(pickAddOnList.map((item) => item.packed ))
    setPicking(prev => prev.concat(dap))
    // if(checked){
          ...
          <Grid container spacing={2}>
            <Grid item>
              <ButtonBase sx={{ width: 10, height: 10 }}>
                <input type="checkbox" value={pick} onChange={() => handlePicking(item, item.packed)}/>
              </ButtonBase>
            </Grid>
