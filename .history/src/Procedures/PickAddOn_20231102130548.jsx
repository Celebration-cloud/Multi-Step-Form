...
const handlePicking = (e, dap) => {
    const {value, checked} = e.target
    if(checked){
      setPicking((prev)=> [...prev, dap])
    }else{
      setPicking((prev) => {return {...prev.filter((item) => item !== value && item)}})
    }
    console.log(picking)
    addOn(picking)
  }
          ...
          <Grid container spacing={2}>
            <Grid item>
              <ButtonBase sx={{ width: 10, height: 10 }}>
                <input type="checkbox" checked={picking.includes(item)} value={item} onChange={() =>handlePicking(item)}/>
              </ButtonBase>
            </Grid>
