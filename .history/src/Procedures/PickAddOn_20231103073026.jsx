...
function PickAddOn({addOn, planner}) {
  const [view, setView] = React.useState(pickAddOnList);
  ...
  const handleTick = (e) => {
    const {value, checked} = e.t
    setSelected(prev => [...prev, check])
    if (selected === true) {
      setView((prev) => [...prev, value])
    } else {
      setView(prev => {return prev.map((item) => item.id === id ? {...item, packed: !item.packed}: item)})
    }
    addOn(view)
  }
  return (
    ...
    <div style={{display: 'flex', justifyContent:'center'}}>
      {pickAddOnList?.map((odds)=> (
        <div key={odds.id}>
          <input type="checkbox"onChange={() => handleTick(odds.packed,odds.id)} value={odds} />
          <div>
            <h6>{odds.name}</h6>
