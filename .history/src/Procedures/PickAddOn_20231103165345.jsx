...
function PickAddOn({addOn, planner}) {
  ...
  const handleTick = (e, dap) => {
    ...
    if (selected === true) {
      ...
      setView(prev => {return prev.map((item) => item.id === dap.id ? {...item, packed: !item.packed}: item)})
    }
    addOn(view)
  }
  return (
    ...
    <div style={{display: 'flex', justifyContent:'center'}}>
      {pickAddOnList?.map((odds)=> (
        <div key={odds.id}>
          <input type="checkbox" checked={selected} onChange={(e) => handleTick(e, odds)} value={odds.packed} />
          <div>
            <h6>{odds.name}</h6>
